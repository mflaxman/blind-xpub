...
## Intro
In this scheme, we demonstrate using large and randomly generated BIP32 path to blind a *single* seed phrase in our multisig quorum.
It works today, under existing hardware wallets, and has implications for both enhanced privacy and trust-minimized collaborative key-holders.
tldr: moar multisig!

This document focuses on the technical specifications of the blinding protocol.
For why this matters, please see [this sub-document](WHY.md).
...
This should work for any Coordinator software that supports modern multisig standards (output descriptors and PSBTs).
We'll also duplicate the code in [buidl](https://github.com/buidl-bitcoin/buidl-python) as it's a no-dependency bitcoin implementaiton written in python that is very easy to use and provides full support for every step of the blinding protocol.
...
[Here is a screenshot of proving accuracy of this derivation path](blinding_from_vpub.png), and [here is a screenshot assuming you have access to their seed phrase](blinding_from_seed_phrase.png) (SLIP132 version byte encoding [here](blinded_tpub_to_vpub.png)).

## Create Account Map

In our case, we take the regular seed phrase A xpub (see [Seed Phrase A](#Seed-Phrase-A) above) and the blinded Seed Phrase B xpub we just calculated (see [Blind 1 Seed Phrase](#Blind-1-Seed-Phrase) above) and combine them into a `1-of-2 p2wsh sortedmulti` using Bitcoin Core (via Specter-Desktop):
...
```
(easier to view colorized screenshot [here](multiwallet_sellx12_blinded_spend.png))
...
**Under this construction, if an unauthorized party gained access to any single seed phrase they'd learn nothing about what it protects.**
